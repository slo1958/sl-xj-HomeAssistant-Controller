...
Begin DesktopWindow Window1
   ...
   Visible         =   True
   Width           =   600
   Begin DesktopButton btn_meta
      AllowAutoDeactivate=   True
      Bold            =   False
      Cancel          =   False
      Caption         =   "Get meta data"
      Default         =   False
      Enabled         =   True
      FontName        =   "System"
      FontSize        =   0.0
   ...
   Begin DesktopTextArea TextArea1
      ...
      Width           =   544
   End
   Begin DesktopListBox lb_Metadata
      AllowAutoDeactivate=   True
...
#tag WindowCode
	#tag Method, Flags = &h0
		Function ExtractSelectedData(selectedID() as string) As clDataPool
		  ...
		  var db as new clExtractPostgres(cnt)
	...
	#tag Method, Flags = &h0
		Function getMetadataRowset() As rowset
		  
		  var cnt as new clHADBConnectionInfo
		  
		  var db as new clExtractPostgres(cnt)
		  
		  var rs as RowSet = db.GetMetaData
	...
	#tag Method, Flags = &h0
		Function getSelectedMetadataID(MetadataIDColumn as integer) As string()
		  
		  var retval() as string
		  
		  for i as integer = 0 to lb_Metadata.LastRowIndex
		    var b as Boolean = lb_Metadata.CellCheckBoxValueAt(i,0)
		    
		    if b then retval.Add(lb_Metadata.CellTextAt(i, MetadataIDColumn))
	...
	#tag Method, Flags = &h0
		Sub WriteMessage(msg as string)
		  ...
		  TextArea1.AddText(msg+EndOfLine)
...
#tag Events btn_meta
	#tag Event
		Sub Pressed()
		  ...
		  var rs as RowSet = getMetadataRowset()
...
#tag Events lb_Metadata
	#tag Event
		Function RowComparison(row1 as Integer, row2 as Integer, column as Integer, ByRef result as Integer) As Boolean
		  ...
		  try
		    var v1 as integer = me.CellTextAt(row1, column).ToInteger
		    var v2 as integer = me.CellTextAt(row2, column).ToInteger
	...
	#tag Event
		Sub Pressed()
		  ...
		  if selection.Count = 0 then
		    WriteMessage("Nothing to extract.")
		    return
		    
		  end if
		  
		  var s as string 
		  if selection.Count = 1 then s = "entity" else s = "entities"
